...
_VAL_DATASET_SPLIT_SIGN = True
_VAL_DATASET_SPLIT_NUM = 10
...
'''
_MEASURE_LATENCY_SAMPLE_TIMES = 10

'''
    The batch size of input data when running forward functions to measure latency.
'''
_MEASURE_LATENCY_BATCH_SIZE = 8

class networkUtils_vgg19(NetworkUtilsAbstract):
    ...
    def __init__(self, model, input_data_shape, dataset_path, finetune_lr=1e-3):
        ...
        if _VAL_DATASET_SPLIT_SIGN:
            val_dataset = self.split_dataset_function(val_dataset, _VAL_DATASET_SPLIT_NUM, self.batch_size)
    ...
    def split_dataset_function(self, dataset, batchnum, batch_size):

        dataset_size = len(dataset)
        indices = list(range(dataset_size))
                
        split_size = batchnum * batch_size
    ...
    def simplify_network_def_based_on_variable(self, network_def, block, remain_ratio):
        return fns.simplify_network_def_based_on_variable(network_def, block, remain_ratio)
    ...
    def compute_weights_and_macs(self, network_def):
        return fns.compute_weights_and_macs(network_def)
...
def vgg19(model, input_data_shape, dataset_path, finetune_lr=1e-3):
